...
public class ControllerExceptionHandler {
  ...
  public ResponseEntity<ErrorMessage> methodArgumentNotValidException(MethodArgumentNotValidException ex, WebRequest request) {
    ErrorMessage message = new ErrorMessage(
            HttpStatus.INTERNAL_SERVER_ERROR.value(),
            new Date(),
            getMessageParameter(ex),
            request.getDescription(false));

    return new ResponseEntity<>(message, HttpStatus.INTERNAL_SERVER_ERROR);
  }

  private String getMessageParameter(MethodArgumentNotValidException ex) {
    if(ex.getFieldError() != null && Objects.requireNonNull(ex.getFieldError()).getDefaultMessage() != null){
